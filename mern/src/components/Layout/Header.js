...
export default function Header() {
  ...
  const renderLogin =()=>{
     let xx = localStorage.getItem('useList')
     var vv = JSON.parse(xx)
     let aa = JSON.parse(localStorage.getItem('useState'))
     if(vv){
      return (
  ...
  const renderCart =()=>{
      var getQty = localStorage.getItem('getValueQty') 
      ...
      return(
        <Link to='/cartlist'><i className="fa fa-shopping-cart" /> Cart {qty}</Link>
      )
  }
  const renderWishList =()=>{
      let getWishList  = localStorage.getItem('getValueWish')
      let getWL =0
      if(getWishList){
        getWL= getWishList
      }
      return(
        <Link to='/wishlist'><i className="fa fa-star" /> WishList {getWL}</Link>
      )
  }
  return (
     <>
