import axios from 'axios'
import React, { useEffect, useState } from 'react'

const CartList = () => {
  const [dataProducts, setDataProduct] = useState([])
  const [dataInput, setDataInput] = useState([])
  useEffect(() => {
        let aa = localStorage['cartlist']
        if(aa){
             let  valueCart = JSON.parse(aa)
             const url = 'http://localhost/laravel/laravel/public/api/product/cart'
             axios.post(url,valueCart)
                .then(res=>{
                  setDataProduct(res.data.data)
                })
                .catch(err=>{console.log(err)})
        }
        
  
  },[])

  const handlePlus =(id)=>{

    let obj = {}
   if(dataProducts.length > 0){
    dataProducts.map(value=>{
        if(value.id === id){
            value.qty++
            setDataInput(value.qty)
        }
        obj[value.id] = value.qty
    })
   }
   localStorage.setItem('cartlist', JSON.stringify(obj))
  }
 
  const handleMines =(id)=>{

     if(dataProducts.length > 0){
      dataProducts.map((value,index)=>{
          if(value.id == id){
              if(value.qty <=1){
                  delete dataProducts[index]
              }else{
                  value.qty--
              }
              let a =[]
              dataProducts.map(value=>{
                  if(value){
                    a.push(value)
                  }
              })
              // console.log(a)
              setDataInput(a)
               
          }
        

      })
    
        
     }
     let obj =   JSON.parse(localStorage.getItem('cartlist')) 
     if(obj){
        Object.keys(obj).map((value,index)=>{
             if(value ==id){
              // console.log(obj[value])
              if( obj[value] <=1){
                
                  delete obj[value]
              }else{

                   obj[value]--
              }
              
             }
        })
     }
    localStorage.setItem('cartlist', JSON.stringify(obj))

  }
  const handleDelete = (id) => {
     
      let getQty=  JSON.parse(localStorage.getItem('getValueQty'))
      let qty=0
      let obj=  JSON.parse(localStorage.getItem('cartlist'))
      if(dataProducts.length >0) {
            dataProducts.map((value,index)=>{
                 if(value.id == id){
                    delete dataProducts[index]
                     delete obj[id]
                     delete getQty[id]
                 }
            })
      }
      let aa =[]
      aa.map((value)=>{
         if(value){
             aa.push(value)
         }
      })
      setDataInput(aa)
      if(getQty){
         getQty = getQty-1
        qty = getQty
      }
      localStorage.setItem('cartlist', JSON.stringify(obj))
      localStorage.setItem('getValueQty', JSON.stringify(qty))



    
  }
   let sum=0
  const renderproductcart =()=>{
    if(dataProducts.length > 0){
      
     return  dataProducts.map(product=>{
        var xx = JSON.parse(product.image)
         sum += product.price * product.qty
         
        return (
          <tr>
            <td className="cart_product">
              <a href>
                <img
                  src={
                    'http://localhost/laravel/laravel/public/upload/user/product/' +
                    product.id_user +
                    '/' +
                    xx[0]
                  }
                  alt=""
                />
              </a>
            </td>
            <td className="cart_description">
              <h4>
                <a href>{product.name}</a>
              </h4>
              <p>Web ID:{product.id} </p>
            </td>
            <td className="cart_price">
              <p>${product.price} </p>
            </td>
            <td className="cart_quantity">
              <div className="cart_quantity_button">
                <a
                  className="cart_quantity_up "
                  onClick={()=>handlePlus(product.id)}
                >
                  {' '}
                  +{' '}
                </a>
                <input
                  className="cart_quantity_input"
                  type="text"
                  name="quantity"
                  defaultValue={1}
                  autoComplete="off"
                  size={2}
                  value={product.qty}
                />
                <a
                  className="cart_quantity_down"
                  onClick={()=>handleMines(product.id)}
                
                >
                  {' '}
                  -{' '}
                </a>
              </div>
            </td>
            <td className="cart_total">
              <p className="cart_total_price">${product.price * product.qty} </p>
            </td>
            <td className="cart_delete">
              <a
                className="cart_quantity_delete"
                 onClick={()=>handleDelete(product.id)}
              >
                <i className="fa fa-times" />
              </a>
            </td>
          </tr>
        )
      })
    }
    
  } 
  return (
    <>
        <section id="cart_items">
        <div className="container">
          <div className="breadcrumbs">
            <ol className="breadcrumb">
              <li>
                <a href="#">Home</a>
              </li>
              <li className="active">Shopping Cart</li>
            </ol>
          </div>
          <div className="table-responsive cart_info">
            <table className="table table-condensed">
              <thead>
                <tr className="cart_menu">
                  <td className="image">Item</td>
                  <td className="description" />
                  <td className="price">Price</td>
                  <td className="quantity">Quantity</td>
                  <td className="total">Total</td>
                  <td />
                </tr>
              </thead>
              <tbody>{renderproductcart()}</tbody>
            </table>
          </div>
        </div>
      </section>{' '}
      {/*/#cart_items*/}
      <section id="do_action">
        <div className="container">
          <div className="heading">
            <h3>What would you like to do next?</h3>
            <p>
              Choose if you have a discount code or reward points you want to
              use or would like to estimate your delivery cost.
            </p>
          </div>
          <div className="row">
            <div className="col-sm-6">
              <div className="chose_area">
                <ul className="user_option">
                  <li>
                    <input type="checkbox" />
                    <label>Use Coupon Code</label>
                  </li>
                  <li>
                    <input type="checkbox" />
                    <label>Use Gift Voucher</label>
                  </li>
                  <li>
                    <input type="checkbox" />
                    <label>Estimate Shipping &amp; Taxes</label>
                  </li>
                </ul>
                <ul className="user_info">
                  <li className="single_field">
                    <label>Country:</label>
                    <select>
                      <option>United States</option>
                      <option>Bangladesh</option>
                      <option>UK</option>
                      <option>India</option>
                      <option>Pakistan</option>
                      <option>Ucrane</option>
                      <option>Canada</option>
                      <option>Dubai</option>
                    </select>
                  </li>
                  <li className="single_field">
                    <label>Region / State:</label>
                    <select>
                      <option>Select</option>
                      <option>Dhaka</option>
                      <option>London</option>
                      <option>Dillih</option>
                      <option>Lahore</option>
                      <option>Alaska</option>
                      <option>Canada</option>
                      <option>Dubai</option>
                    </select>
                  </li>
                  <li className="single_field zip-field">
                    <label>Zip Code:</label>
                    <input type="text" />
                  </li>
                </ul>
                <a className="btn btn-default update" href>
                  Get Quotes
                </a>
                <a className="btn btn-default check_out" href>
                  Continue
                </a>
              </div>
            </div>
            <div className="col-sm-6">
              <div className="total_area">
                <ul>
                  <li>
                    Cart Sub Total <span>$59</span>
                  </li>
                  <li>
                    Eco Tax <span>$2</span>
                  </li>
                  <li>
                    Shipping Cost <span>Free</span>
                  </li>
                  <li>
                    Total <span>${sum}</span>
                  </li>
                </ul>
                <a className="btn btn-default update" href>
                  Update
                </a>
                <a className="btn btn-default check_out" href>
                  Check Out
                </a>
              </div>
            </div>
          </div>
        </div>
      </section>
      {/*/#do_action*/}
    </>
  )
}

export default CartList
